...
//    protected $limitFolder = '/images';
    ...
    public function saveElement()
    {
        ...
        return array('success' => isset($result[0]) ? $result[0] : false, 'data' => $this->getAllElement());
    }
...
END;
    ...
    public function sendEmail()
    {
        ...
        $exclusive  = array('recipient', 'subject', 'from_name', 'from_email', 'custom_message', JFactory::getSession()->getFormToken());
        ...
        if (count((array) $post))
        {
            ...
            foreach ($post as $key => $item)
            {
                if (!in_array($key, $exclusive))
                {
                    if (is_array($item))
                    {
                        if (count((array) $item))
                        {
                            ...
                            foreach ($tmpItem as $subKey => $subItem)
                            {
                                ...
                                {
                                    $item .= '<li><strong>' . ucwords(str_replace('_', ' ', (string) $subKey)) . '</strong>: ' . (string) $subItem . '</li>';
                                }
                                else
                                {
                                    $item .= '<li>' . (string) $subItem . '</li>';
                                }
                            }
                    ...
                    if ((string) $item != '')
                    {
                        $body .= '<p><strong>' . str_replace('_', ' ', (string) $key) . '</strong>: ' . (string) $item . '</p>';
                    }
                }
            }
