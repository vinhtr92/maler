...
class JsnExtFwHelper
{
	...
	public static function getSettings($component = null, $allParams = false)
	{
		...
		if (!isset($settings) || !isset($settings[$component]))
		{
			...
			if ($component === 'jsnextfw')
			{
				$config = JPATH_ROOT . '/plugins/system/jsnextfw/config';
			}
			else
			{
				$config = JPATH_ADMINISTRATOR . "/components/{$component}/config";
			}

			if (JFile::exists("{$config}.json"))
			{
				$settings[$component] = array_merge(self::getDefaultSettings(json_decode(file_get_contents("{$config}.json"), true)),
					$settings[$component]);
			}
			elseif (JFolder::exists($config))
			{
				foreach (JFolder::files($config, '\.json$', true, true) as $config)
				{
					$settings[$component] = array_merge(self::getDefaultSettings(json_decode(file_get_contents($config), true)),
						$settings[$component]);
				}
			}
		}
	...
	public static function getSettingFilters($form = null)
	{
		...
		if (!is_array($form))
		{
			...
			if (JFile::exists("{$config}.json"))
			{
				$form = json_decode(file_get_contents("{$config}.json"), true);
			}
			elseif (JFolder::exists($config))
			{
				foreach (JFolder::files($config, '\.json$', true, true) as $config)
				{
					$form = array_merge_recursive(json_decode(file_get_contents($config), true), $form);
				}
			}
		}
